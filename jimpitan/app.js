...
(function () {
    ...
    const DEFAULT_SCRIPT_URL = 'https://script.google.com/macros/s/AKfycbyBuPaWCQCPjtjzRB8G8nThkCvfPgV3Af_IVa9n_luaJPoZTr2ja5Q-BEdkYShdl9s6/exec';
    ...
    function generateWeeklyShareText() {
        ...
        const tipeLabel = isJimpitan ? 'Pemasukan' : 'Pengeluaran';
        const tipeFilter= isJimpitan ? 'Pemasukan' : 'Pengeluaran';
        ...
        const lines = items.length > 0
            ? items.map(d => {
                ...
                const ket = d.keterangan !== '-' ? ` _(${d.keterangan})_` : '';
                return `🔹 *${dayName}*, ${d.dateObj.getDate()} ${monthShort}
   └ ${formatRp(d.nominal)}${ket}`;
              }).join('\n')
            : '  _(Tidak ada transaksi)_';

        const emoji = isJimpitan ? '💰' : '💸';

        return `*LAPORAN ${tipeLabel.toUpperCase()} MINGGUAN* ${emoji}
🏡 *Bumi Neikarta 2*
📅 ${rangeStr} (Minggu ${weekNum})
${'━'.repeat(15)}

${lines}

${'━'.repeat(15)}
*TOTAL MINGGU ${weekNum}: ${formatRp(total)}*

_Generated by Jimpitan BN2 App 🚀_`;
    }
    ...
    function generateMonthlyShareText() {
        ...
        const activeDays = new Set(state.filteredData.map(d => d.dateObj.toDateString())).size;
        ...
        return `*REKAPITULASI KAS BULANAN* 📊
🏡 *Bumi Neikarta 2*
📅 Periode: *${monthName} ${year}*
${'━'.repeat(15)}

📥 *Total Pemasukan* : ${formatRp(monthIn)}
📤 *Total Pengeluaran*: ${formatRp(monthOut)}
💎 *Saldo Akhir*     : *${formatRp(saldo)}*

📈 _Aktifitas: ${activeDays} hari tercatat_
${'━'.repeat(15)}
...
${'━'.repeat(15)}
_Laporan ini dibuat otomatis melalui aplikasi Jimpitan BN2_`;
    }
    ...
    function renderAll() {
        try {
            ...
            if (statusCard) {
                statusCard.classList.add('status-card-white');
            }
                                ...
                                <div class="bg-white border border-slate-50 p-4 rounded-lg flex items-center gap-4 hover:border-emerald-100 transition-all active:scale-[0.99] cursor-pointer shadow-sm shadow-slate-200/20" onclick="window.showDetailById(${item.idx})">
                                    ...
                                    <div class="flex flex-col items-end gap-1">
                                        ...
                                        <div class="flex gap-1 mt-1">
                                            ...
                                            <button onclick="event.stopPropagation(); window.deleteEntry(event, ${item.idx})" class="w-10 h-10 flex items-center justify-center bg-rose-50 rounded-lg text-rose-500 active:bg-rose-100" title="Hapus">
    ...
    function bindModals() {
        ...
        window.showDetailById = (idx) => {
            const item = state.data.find(d => d.idx === idx);
            if (item) {
                const isJimpitan = item.tipe === 'Pemasukan';
                ...
                if (adminActions) {
                    if (state.isAdmin) {
                        ...
                        if (deleteBtn) deleteBtn.onclick = (e) => window.deleteEntry(e, item.idx);
        ...
        window.handleEdit = (item) => {
            ...
            // Konversi tanggal yang lebih aman menggunakan dateObj
            ...
            if (!isNaN(d.getTime())) {
                ...
                // Fallback jika dateObj bermasalah
                const today = new Date().toISOString().split('T')[0];
                $('#entryDate').value = today;
            }
        ...
        // Fungsi Global Delete (mendukung loading spinner pada tombol yang diklik)
        window.deleteEntry = (event, idx) => {
            if (confirm('Apakah Anda yakin ingin menghapus data ini?')) {
                const clickedBtn = event.currentTarget;
                deleteEntry(idx, clickedBtn);
            }
        };
