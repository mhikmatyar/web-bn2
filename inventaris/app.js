        ...
        tbody.innerHTML = pageItems.map(item => {
            ...
            return `
                <tr>
                    ...
                    <td data-label="Merk/Type">${escapeHtml(item.merkType)}</td>
                    <td data-label="Tahun">${escapeHtml(item.tahunPerolehan)}</td>
                    <td data-label="Jumlah">${item.jumlah}</td>
                    <td data-label="Harga Satuan" class="harga-col">${item.hargaSatuan > 0 ? 'Rp ' + item.hargaSatuan.toLocaleString('id-ID') : '-'}</td>
